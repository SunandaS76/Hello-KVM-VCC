	...
	for (;;)
	{
		...
		case KVM_EXIT_IO:
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xE9)
			{
				...
				// printf("%d", *(uint32_t *)(p + vcpu->kvm_run->io.data_offset) );
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xE9)
			{
				...
				char *p = (char *)vcpu->kvm_run;
				*(p + vcpu->kvm_run->io.data_offset) = exits;
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEA)
			{
				...
				translation.linear_address = mem;
				...
				printf("%s", vm->mem + translation.physical_address);
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT && vcpu->kvm_run->io.port == 0xEC)
			{
				...
				memcpy(&mem_1, p + vcpu->kvm_run->io.data_offset, vcpu->kvm_run->io.size);

				translation.linear_address = mem_1;
				if (ioctl(vcpu->fd, KVM_TRANSLATE, &translation) < 0)
				{
					perror("VM_TRANSLATE");
					exit(1);
				...
				if (translation.valid == 1){
					...
					printf("GVA is valid, corresponding HVA is: ");

					mem = (uintptr_t) (vm->mem + translation.physical_address);
					// printf("inside vmm memory access 4 %d\n", mem);
				...
				// mem = (vm->mem + translation.physical_address);
				continue;
			}
				...
				if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN && vcpu->kvm_run->io.port == 0xEC)
				{
					exits++;
					char *p = (char *)vcpu->kvm_run;
					*(uint32_t *)(p + vcpu->kvm_run->io.data_offset) = mem;
					continue;
				}
